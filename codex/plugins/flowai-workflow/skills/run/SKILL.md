...
ships the engine TypeScript source under `./engine/`
and the canonical bundled workflows under
`./.flowai-workflow/<name>/`. Runs use the host's
...
2. If the argument matches a sibling under
   `./.flowai-workflow/`, use that bundled folder.
...
FLOWAI_SUPPRESS_DEPRECATION=1 \
  deno run -A "./bin/launch.ts" run "<resolved-workflow-path>" [extra args]
```
...
- "Permission denied: …" — the engine writes under
  ...
  workflow lives under `./.flowai-workflow/` (which
